...
export async function reserveStockUseCase(
  ...
  return recordMovementUseCase(itemRepository, stockLevelRepository, movementRepository, {
    ...
    reason: params.reason ?? "Резерв склада",
    idempotencyKey: `reservation_hold:${params.referenceId ?? "manual"}:${params.itemId}:${params.quantity}`,
